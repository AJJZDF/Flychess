...
//    public static final int FOURP = 4;//4p
    ...
    //测试一下是不是可以再加一个玩家(人类玩家)到房间,主要是用户点击在线房间列表时判断下是不是可以进入,不允许含有重名玩家
    ...
    public Room(String roomname,String roomowner)
    {
        ...
        for(int i = 0 ; i < 4; i++)
        {
            roommateStates[i] = new NetworkRoommateState();
    ...
    public static void main(String[] args) throws UnknownHostException {
        ...
        InetAddress mainIP = InetAddress.getLocalHost();
        InetAddress ip1 = InetAddress.getByName("127.1.2.1");
        InetAddress ip2 = InetAddress.getByName("127.1.2.2");
        InetAddress ip3 = InetAddress.getByName("127.1.2.3");
        InetAddress ip4 = InetAddress.getByName("127.1.2.4");
        String name1 = "player1";
        String name2 = "player2";
        String name3 = "player3";
        String name4 = "player4";

        Room room = new Room("gg","gitfan");
        if(room.addPlayer(mainIP,"gitfan")) System.out.println("good 1");

        if(room.join_test(name1))
        {
            if(room.addPlayer(ip1,name1)) System.out.println("good 2");
        }

        if(room.pos_removeAI(mainIP,1)) System.out.println("good 7");

        if(room.join_test(name2))
        {
            if(room.addPlayer(ip2,name2)) System.out.println("good 3");
        }
        if(room.join_test(name3))
        {
            if(room.addPlayer(ip3,name3)) System.out.println("good 4");
        }
        if(room.join_test(name4))
        {
            if(room.addPlayer(ip4,name4)) System.out.println("good 5");
        }
    }
}
