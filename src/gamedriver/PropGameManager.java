...
public class PropGameManager extends GameManager{
    ...
    private int bear_prop = Prop.NO_PROP;//当前承受的道具(比如遇到龙卷风,栅栏...,拾取道具不算进去)
    ...
    public Queue<Prop> getProps()
    {
        ...
        bear_prop = Prop.NO_PROP;//恢复...bear_prop
        randomPos();
        return props;
    }
    //为某个玩家添加道具
    ...
    //返回true表示要停止
    private boolean exec_prop(boolean end)
    {
        ...
        if(prop_board == Prop.BARRIER)
        {
            ...
            bear_prop = prop_board;

            //如果是类似保护罩之类的东西
            if(curr_prop[getTurn()] == Prop.PROTECTION) return false;
            else return true;
        }
        //如果是拾取类的东西
        ...
        if(end)
        {
            if(prop_board == Prop.TORNADO)//龙卷风
            {
                ...
                curr_action_list.enqueue(action);
                propboard[chess.getPos()].setProp(Prop.NO_PROP);
                bear_prop = prop_board;//需不需更新bear_prop视情况而定
    ...
    private boolean exec_single_move()
    {
        ...
        boolean isStop = exec_prop(false);
    ...
    private boolean exec_last_move()
    {
        ...
        {
            exec_eat(chessboard[chess.getPos()],true);
            exec_turnright();
            return false;
        }
        ...
        {
            exec_merge(chessboard[chess.getPos()],true);
            return false;
        }

        boolean isStop = exec_prop(true);
    ...
    private void exec_eat(Chess that,boolean update)
    {
        Action action;
        ...
        if(update) chessboard[that.getPos()] = new Chess(player[getTurn()].chesslist[choice]);
    }
    private void exec_merge(Chess that,boolean update)
    {
        Chess chess = player[getTurn()].chesslist[choice];
        Action action;
        for(Pair pair:that.getIndexlist())
        ...
        that.clear_chess();
        if(update) chessboard[chess.getPos()] = new Chess(chess);
    }
    ...
    private void exec_move(int step)
    {
        ...
        if(chess.isLucky() && !chess.isSuperLucky() && chess.getPos() != chess.getEntry())
        {
    ...
    private Queue<Action> prop_action_list()
    {
        ...
        if(curr_prop[getTurn()] == Prop.NO_PROP)
        {
            ...
            else if(chess.presprint(dice))
            {
                ...
                else if(chess.mergeTest(chessboard[chess.getPos()]))
                {
                    exec_merge(chessboard[chess.getPos()],true);
                }
            }
...
//        chess.setStatus(Chess.STATUS_FLYING);
//        chess.setPos(46);
//        gameManager.chessboard[46] = new Chess(chess);
